...
describe("LINE rich-message boundaries", () => {
  ...
  it("drops the reply-to LINE has no primitive for", async () => {
    ...
    expect(
      (await prepareLineReplyPayload({ text: "Done.", replyToId: "630776817589944423" }))
        .replyToId,
    ).toBeUndefined();
    expect(
      (
        await prepareLineReplyPayload({
