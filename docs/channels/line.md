...
- `requireMention` applies to text by default. LINE puts mention data only on text, so a photo, video, audio message, file or location can never address the bot, and a sticker can only by quoting one of the bot's own messages. Applying the requirement to them would stop groups that rely on answered attachments. Set `channels.line.groups.<groupId>.requireMentionOnAllMessageTypes: true` to extend it to every kind, which is how the requirement behaves on other channels.
- With that opt-in set, an unaddressed attachment is kept as room context instead of answered. A kept photo is available to the mention that follows it — that one mention, after which it is consumed — and only while the Gateway keeps running, since the kept window lives in the process. Video, audio and file messages are recorded by what they are rather than by their contents, so their contents cannot reach the agent in that group at all; leave the opt-in unset there, or set `requireMention: false`, which answers every message in the group. A photo the bot could not fetch says so in the room context instead of reading as one it can still see.
