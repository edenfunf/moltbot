...
- A group reply made with LINE's quote gesture carries the quoted message's author, and its text where the message had text, into the turn, so the agent can answer "what about this?" without guessing. LINE's webhook reports only the quoted message's ID, so the text is resolved from what this conversation already admitted: the most recent few hundred messages per account, kept in memory and scoped to the conversation they arrived in. That store is larger than the chat window the agent sees, so a quote can resolve to a message that has already scrolled out of it. A quote of anything older, of a message from another chat, of a group message that only ever sat in the ambient window while that window is switched off (`historyLimit: 0`), or of anything at all after a Gateway restart reaches the agent as a bare reference with no author and no body under the default `contextVisibility: "all"` and under `"allowlist_quote"`; under `"allowlist"`, in a group that actually has an allowlist to check, an unresolved quote has no author to clear, so the reference is dropped with it, and a Gateway run with `--verbose` logs each drop as `drop quoted context`. A quote of one of the bot's own messages carries its author but no body: the store holds no outbound text, so the quote is named as the assistant's own without saying which reply it was. Every message this account sends is recognized that way, including ones sent from outside the conversation, such as a cron job or `openclaw message send`, so the agent has only the conversation to place it by. That author is also what tells it apart from a quote this account can no longer resolve, which arrives with no author at all. A quote of a photo, video, or file comes back as a short marker such as `<image>`, and a quote of a sticker or location as the text the agent is given for it: the sticker's description, or the formatted location. Attachments themselves are never retained. The ambient window uses the same text, so a sticker or location posted in a group without addressing the bot, a shared location's coordinates included, reaches the agent the next time someone does address it. A message is kept as it arrived, so a quote of one that was later edited or unsent returns the original text. A long quoted message keeps its head and tail with `…[omitted]…` between them.
