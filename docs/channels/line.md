...
- `requireMention` applies to text by default. LINE puts mention data only on text, so a photo, video, audio message, file, sticker or location can never address the bot, and applying the requirement to them would stop groups that rely on answered attachments. Set `channels.line.groups.<groupId>.requireMentionOnAllMessageTypes: true` to extend it to every kind, which is how the requirement behaves on other channels.
